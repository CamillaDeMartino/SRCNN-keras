...
if __name__ == "__main__":
    im1 = cv2.imread(str(ASSETS_DIR / "input.jpg"), cv2.IMREAD_COLOR)
    im1 = cv2.cvtColor(im1, cv2.COLOR_BGR2YCrCb)[6: -6, 6: -6, 0]
    im2 = cv2.imread(str(ASSETS_DIR / "butterfly_GT.bmp"), cv2.IMREAD_COLOR)
    im2 = cv2.cvtColor(im2, cv2.COLOR_BGR2YCrCb)[6: -6, 6: -6, 0]
    im3 = cv2.imread(str(ASSETS_DIR / "pre_adam30.jpg"), cv2.IMREAD_COLOR)
    im3 = cv2.cvtColor(im3, cv2.COLOR_BGR2YCrCb)[6: -6, 6: -6, 0]
    im4 = cv2.imread(str(ASSETS_DIR / "pre2.jpg"), cv2.IMREAD_COLOR)
